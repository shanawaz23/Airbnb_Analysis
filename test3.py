...
st.markdown("<h1 style='text-align: center;'>AIRBNB DATA ANALYSIS</h1>", unsafe_allow_html=True)

def datafr():
    df= pd.read_csv("C:/Users/shana/OneDrive/Desktop/New folder/.venv/Airbnb.csv")
    return df

df= datafr()

select = option_menu(None , ["INSTRUICTIONS & HOME", "DATA VISUALISATION", "Complete Analysis"],
...
if select == "DATA VISUALISATION":
    ...
    with tab2:
        ...
        def datafr():
            df_a= pd.read_csv("C:/Users/shana/OneDrive/Desktop/New folder/.venv/Airbnb.csv")
            return df_a
        ...
        with col1:
            ...
            df_sunburst = df2_a.groupby(['room_type', 'bed_type', 'is_location_exact']).agg({'availability_30': 'sum'}).reset_index()


            df_a_sunb_30= px.sunburst(df_sunburst, path=["room_type","bed_type","is_location_exact"], values="availability_30",width=600,height=500,title="Availability_30",color_discrete_sequence=px.colors.sequential.Peach_r)
        ...
        with col2:
            ...
            df_sunburst_1 = df2_a.groupby(['room_type', 'bed_type', 'is_location_exact']).agg({'availability_60': 'sum'}).reset_index()
            df_a_sunb_60= px.sunburst(df_sunburst_1, path=["room_type","bed_type","is_location_exact"], values="availability_60",width=600,height=500,title="Availability_60",color_discrete_sequence=px.colors.sequential.Blues_r)
            st.plotly_chart(df_a_sunb_60)

        col1,col2= st.columns(2)

        with col1:
            df_sunburst_2 = df2_a.groupby(['room_type', 'bed_type', 'is_location_exact']).agg({'availability_90': 'sum'}).reset_index()
            df_a_sunb_90= px.sunburst(df_sunburst_2, path=["room_type","bed_type","is_location_exact"], values="availability_90",width=600,height=500,title="Availability_90",color_discrete_sequence=px.colors.sequential.Aggrnyl_r)
            st.plotly_chart(df_a_sunb_90)

        with col2:
            df_sunburst_3 = df2_a.groupby(['room_type', 'bed_type', 'is_location_exact']).agg({'availability_365': 'sum'}).reset_index()
            df_a_sunb_365= px.sunburst(df_sunburst_3, path=["room_type","bed_type","is_location_exact"], values="availability_365",width=600,height=500,title="Availability_365",color_discrete_sequence=px.colors.sequential.Greens_r)
